...
import { getCurrentPersonalityId, getPersonalityDirectory } from '../memory/personality-manager';
...
function extractCoreRules(sohaContent: string): string {
  const lines = sohaContent.split('\n');
  const coreRules: string[] = [];
  let inRules = false;
  let rulesCount = 0;

  for (const line of lines) {
    if (line.includes('##') && (line.includes('说话风格') || line.includes('硬性规则') || line.includes('专属词汇'))) {
      inRules = true;
    }
    if (inRules && rulesCount < 50) {
      coreRules.push(line);
      rulesCount++;
    }
    if (line.includes('##') && inRules && !line.includes('说话风格') && !line.includes('硬性规则') && !line.includes('专属词汇')) {
      break;
    }
  }

  return coreRules.join('\n') || buildDefaultNahidaPrompt();
}

function buildDefaultNahidaPrompt(): string {
...
function appendHistory(sessionId: string, msg: ChatMessage, cycleLog?: CycleLogEntry[]): void {
  ...
  persistMessage(sessionId, msg.role, msg.content, cycleLog);
}
